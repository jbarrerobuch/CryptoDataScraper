...
def write_marketdata_in_db(is_active:bool=True, is_complete:bool=False) -> None:
    """
    ...
    instruments = db_methods.read_last_date_from_instruments(is_active=is_active, is_complete=is_complete)
    ...
    for index,row in instruments.iterrows():
        ...
        if instrument_count >= 100 and len(data_to_write) > 0:
            ...
            status, writen_rows, failed_rows = db_methods.write_df_to_db(data=data_to_write, table_name="market_data", print_msg=f"{data_to_write['instrument_name'].unique()}")
